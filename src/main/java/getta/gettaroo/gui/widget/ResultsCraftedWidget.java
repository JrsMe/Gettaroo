...
import net.minecraft.registry.Registries; // Actualización aquí
...
public class ResultsCraftedWidget extends WidgetBase {
    ...
    private static String getItemStackName(ItemStack item) {
        // Mejorar la obtención del nombre de forma más robusta
        return item.getItem().getName(item).getString().toLowerCase().replace(" ", "_");
    }
    ...
    public void render(int mouseX, int mouseY, boolean selected, MatrixStack matrixStack) {
        ...
        if (showResults && !this.results.isEmpty()) {
            ...
            for (CraftingPanelItemOutput item : items) {
                ...
                String identifier = "minecraft:" + getItemStackName(Registries.ITEM.get(new Identifier("minecraft:" + item.getName().replace(" ", "_").toLowerCase()))).toLowerCase();
                ItemStack itemStack = Registries.ITEM.get(new Identifier(identifier)).getDefaultStack();
